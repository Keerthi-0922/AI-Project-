...
This project implements a simple and efficient image forgery detection system using frequency 
domain analysis (FFT) and a Convolutional Neural Network (CNN). The model is trained on synthetically 
generated image data to distinguish between real and forged images.

**Features**

1.Converts images to frequency domain using Fast Fourier Transform (FFT)

2.Uses a custom CNN model for binary classification (Real vs Forged)

3.Allows synthetic data generation for training

4.Accepts user-uploaded images for prediction

5.Visualizes FFT spectrum and prediction results
...
Use the file upload prompt (if using Colab) or modify the script to load local images.
